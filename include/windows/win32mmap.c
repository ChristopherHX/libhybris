...
void *mmap(void *start, size_t length, int prot, int flags, int fd, off_t offset)
{
	...
	hmap = CreateFileMapping(osfhandle, NULL,
		prot & PROT_READ ? (prot & PROT_EXEC ? PAGE_EXECUTE_READ : PAGE_READONLY) : (prot & PROT_EXEC ? PAGE_EXECUTE_WRITECOPY : PAGE_WRITECOPY), 0, 0, NULL);
	...
	temp = MapViewOfFileEx(hmap, prot & PROT_READ ? (prot & PROT_EXEC ? FILE_MAP_READ | FILE_MAP_EXECUTE : FILE_MAP_READ) : (prot & PROT_EXEC ? FILE_MAP_COPY | FILE_MAP_EXECUTE : FILE_MAP_COPY), h, l, length, start);
